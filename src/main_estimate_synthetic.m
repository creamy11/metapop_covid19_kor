...
for i = 1:n_test_param
    suffix_span = [suffix_span, sprintf('_%d', i)];
end

result_path = '../result/estimation/synthetic';
...
E_is_inflated = true(n_loc);
E_is_reprobed = true(n_loc);

Ir0_lb = zeros(n_loc, n_loc);
Ir0_ub = zeros(n_loc, n_loc);
Ir_is_inflated = true(n_loc);
Ir_is_reprobed = true(n_loc);

Iu0_lb = zeros(n_loc, n_loc);
Iu0_ub = diag(5 * ones(n_loc, 1));
Iu_is_inflated = true(n_loc);
Iu_is_reprobed = true(n_loc);

S0_lb = round(0.9 * N0);
S0_ub = N0 - E0_ub - Ir0_ub - Iu0_ub;% Make sure the sum does not exceed N
S_is_inflated = true(n_loc);
S_is_reprobed = true(n_loc);

Q0_lb = zeros(n_loc, n_loc);
Q0_ub = Q0_lb;
Q_is_inflated = true(n_loc);
Q_is_reprobed = true(n_loc);
...
% Ensemble
n_ens = 300;
n_real = 10;% Different initial ensembles from sampling
rng(0)% Meaningless if you use parfor
...
if beta_is_local
    n_loc_var = (n_state_type - 1) * n_loc + 3;
    global_idx = alpha_idx;%[mu_idx, Z_idx, alpha_idx, Dr_idx, Du_idx, G_idx];
else
...
for ii = 1:n_test_param

    fprintf('Running %dth test...\n', ii)

    suffix = suffix_span{ii};

    % Load synthetic incidence data
    load(sprintf('../pipeline/generate_synthetic_data/data%s.mat', suffix), ...
        'truth_data', 'obs_data', 'obs_error_var', '*_truth')

    % Smooth observation data
    obs_data_smoothed = movmean(obs_data, 7, 2);

    % Determine terminal time and the number of observations to use
    truth_data_sum = sum(truth_data, 1);
    non_zero_case_idx = find(truth_data_sum > 0);
    if isempty(non_zero_case_idx)
        t_termi = size(obs_data_smoothed, 2);% When R_0 < 1
    else
        t_termi = non_zero_case_idx(end);
    end
    tspan_obs = t_init:dt_obs:t_termi;
    n_obs = length(tspan_obs) - 1;
    ...
    for i = 1:n_obs
        ...
        n_day = length(dt_init:dt_termi);
        tspan = zeros(1, 2 * n_day - 1);
        tspan(1) = dt_init;
        for j = 1:n_day - 1
    ...
    for jj = 1:n_real
        ...
        fprintf('%dth realization is running...\n', jj)

        %% Generate Initial Ensemble Memebers
        ...
        for i = 1:n_obs
            ...
            for j = 1:n_loc
                ...
                obs_var = max(5, (obs_data_smoothed(j, i) ^ 2) / 5);
                ...
                post_mean = post_var * (prior_mean / prior_var + obs_data_smoothed(j, i) / obs_var);
        ...
        x_post_state(:, :, :, jj) = reshape(sum(X_post_state, 1), n_loc * n_state_type, n_ens, n_obs);

        % Save posterior distribution of observations
        x_post_obs(:, :, :, jj) = x_post(n_state_total + (1:n_loc), :, :);

        % Save posterior distribution of unreported daily cases
        x_post_incidence_u(:, :, :, jj) = x_post(n_state_total + n_loc + (1:n_loc), :, :);

        % Save posterior distribution of parameters
        x_post_parameter(:, :, :, jj) = x_post(n_state_total + 2 * n_loc + (1:n_parameter), :, :);

    end

    fprintf('Saving results with suffix name: %s\n', suffix)
    save(sprintf('%s/estimate%s_fix.mat', result_path, suffix), '*_lb', '*_ub', ...
        't_init', 't_termi', 'dt_obs', 'n_state_total', 'obs_data_smoothed', ...
        'x_post_state', 'x_post_obs', 'x_post_incidence_u', 'x_post_parameter', '-v7.3')
